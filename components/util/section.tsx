...
export const Section = ({ children, color = '', className = '' }) => {
  ...
  const sectionColorCss =
    color === 'primary' ? sectionColor.primary[theme.color] : sectionColor[color] ? sectionColor[color] : sectionColor.default;

  return (
    <section className={`flex-1 relative transition duration-150 ease-out body-font overflow-hidden ${sectionColorCss} ${className}`}>
      {children}
    </section>
  );
};
